...
export const OFFICE_CAMERA = { position: [28, 32, 40] as const, target: [0, 1, -2] as const };
export class Stage {
  ...
  constructor(rendererElement: HTMLElement) {
    ...
    this.camera=new THREE.PerspectiveCamera(38,1,.1,250);
    ...
    this.controls.minDistance=5;this.controls.maxDistance=120;
    ...
    this.waypointIndicator=new THREE.Mesh(ring,new THREE.MeshBasicNodeMaterial({color:0xb99b63,transparent:true,opacity:.8}));
  ...
  public onResize(width:number,height:number){this.camera.aspect=width/Math.max(height,1);this.camera.updateProjectionMatrix();}
  public setFollowTarget(pos:THREE.Vector3|null){this.followTarget=pos?.clone()??null;}
  public resetView(){this.followTarget=null;this.camera.position.set(...OFFICE_CAMERA.position);this.controls.target.set(...OFFICE_CAMERA.target);this.controls.update();}
